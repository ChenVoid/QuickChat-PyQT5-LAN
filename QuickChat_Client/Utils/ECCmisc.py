...
def get_key(ip):
    ...
    # user1生成公钥，大KEY
    # 注意这里的公钥KEY_x、KEY_y需要传给对方！
    KEY_x, KEY_y = get_ng(G_x, G_y, key, a, p)
    return [key,KEY_x,KEY_y]


def ECC_encrypt(input,KEY_x,KEY_y):
    a = 100
    b = 456
    p = 17
    G_x = 5
...
def ECC_decrypt(key,c):
    ...
    for charArr in c:
        ...
        str = str + chr(charArr[2] // decrypto_text_x)
    return str
...
if __name__ == "__main__":
    ...
    ip = ip_str2int(ip)
    key=get_key(ip)
#    cipher_text = ECC_encrypt(key[1], key[2])
#    print(cipher_text)
    #ECC_decrypt(key[0], cipher_text)
    print(ECC_decrypt(5, 1))
